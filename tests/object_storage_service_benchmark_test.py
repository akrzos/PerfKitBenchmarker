...
class TestBuildCommands(unittest.TestCase):
  ...
  def testBuildCommands(self):
    vm = mock.MagicMock()
    vm.RemoteCommand = mock.MagicMock(return_value=('', ''))
    ...
    self.assertEqual(
        vm.RemoteCommand.call_args_list[0],
    ...
    self.assertEqual(
        vm.RemoteCommand.call_args_list[1],
        mock.call('test_script.py --storage_provider=GCP --bucket=bucket '
 ...
 {"latency": 0.1, "operation": "download", "stream_num": 1, "start_time": 10.0, "size": 1}]"""  # noqa: line too long
  ...
  def testParseResults(self):
    ...
    self.assertEqual(samples,
                     [sample.Sample('Multi-stream 1 KB upload',
                                    0.1,
                                    'sec',
                                    {'stream_num': 1, 'object_size_b': 1},
                                    5.0),
                      sample.Sample('Multi-stream 1 KB download',
                                    0.1,
                                    'sec',
                                    {'stream_num': 1, 'object_size_b': 1},
                                    10.0)])
...
class TestStringToBytes(unittest.TestCase):
  def testValidString(self):
    self.assertEqual(object_storage_service_benchmark._StringToBytes('100KB'),
                     100000)

  def testUnparseableString(self):
    with self.assertRaises(ValueError):
      object_storage_service_benchmark._StringToBytes('asdf')

  def testBadUnits(self):
    with self.assertRaises(ValueError):
      object_storage_service_benchmark._StringToBytes('100m')

  def testNonIntegerBytes(self):
    with self.assertRaises(ValueError):
      object_storage_service_benchmark._StringToBytes('1.5B')

  def testNegativeBytes(self):
    with self.assertRaises(ValueError):
      object_storage_service_benchmark._StringToBytes('-10KB')


class TestStringToRawPct(unittest.TestCase):
  def testValidPct(self):
    self.assertEquals(object_storage_service_benchmark._StringToRawPct('50.5%'),
                      50.5)

  def testNullString(self):
    with self.assertRaises(ValueError):
      object_storage_service_benchmark._StringToRawPct('')
  ...
  def testNoPercentSign(self):
    with self.assertRaises(ValueError):
      object_storage_service_benchmark._StringToRawPct('100')

  def testNegativePercent(self):
    with self.assertRaises(ValueError):
      object_storage_service_benchmark._StringToRawPct('-12%')

  def testPercentAbove100(self):
    with self.assertRaises(ValueError):
      object_storage_service_benchmark._StringToRawPct('112%')
