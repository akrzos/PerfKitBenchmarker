...
flags.DEFINE_string('object_sizes', 1, 'The size of the objects to use, as a '
                    ...
                    'MultiStreamThroughput scenario, ignored in others.')
...
class SizeDistributionIterator(object):
  ...
  def __iter__(self):
    return self  # this is required by the Python iterator protocol
  ...
  for i in xrange(count):
    ...
    try:
      _, latency = WriteObjectFromBuffer(
          storage_schema, bucket, object_prefix,
...
def WriteObjectFromBuffer(storage_schema, bucket, object,
                          payload_bytes, payload_string, size,
                          host_to_connect=None):
  ...
  Args:
    ...
    size: The size of the objects to write, in bytes. If size is
  ...
  if _useBotoApi(storage_schema):
    object_path = '%s/%s' % (bucket, object)
    object_uri = boto.storage_uri(object_path, storage_schema)
    if host_to_connect is not None:
      object_uri.connect(host=host_to_connect)

    object_uri.set_contents_from_string(payload_string[:size])
  else:
    _AZURE_BLOB_SERVICE.put_block_blob_from_bytes(bucket, object,
...
def ReadObject(storage_schema, bucket, object, host_to_connect=None):
  """Read an object.
  ...
  if _useBotoApi(storage_schema):
    ...
    _AZURE_BLOB_SERVICE.get_blob_to_bytes(bucket, object)
...
def MultiStreamWrites(storage_schema, host_to_connect=None):
  """Run multi-stream write threads."""
  ...
  # it can read. The file format is a JSON list of lists in the format
...
def MultiStreamReads(storage_schema, host_to_connect=None):
  ...
  objects_by_worker = []
  objects_per_worker = (len(object_records) + num_workers - 1) // num_workers
  for i in xrange(num_workers):
    # Do a simple, even distribution of objects to workers.
    low = i * objects_per_worker
    high = min((i + 1) * objects_per_worker, len(object_records))
    objects_by_worker.append(object_records[low:high])
...
def WriteWorker(storage_schema, host_to_connect,
  ...
  object_prefix = ('pkb_write_worker_%s' % worker_num)
...
def ReadWorker(storage_schema, host_to_connect,
  ...
  for name, size in object_records:
    try:
      ...
      logging.info('Caught exception %s while reading object %s' %
                   (e, name))
